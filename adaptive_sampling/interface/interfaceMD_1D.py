...
class MD:
    ...
    def __init__(
        ...
        self.forces = np.zeros(self.natoms)
        self.momenta = np.zeros(self.natoms)
        ...
        # Mass
        self.mass = mass_in
        self.conf_forces = np.zeros(self.natoms)
        self.conf_traj = []
        self.masses = np.full(1, self.mass)
    ...
    def calc(self) -> Tuple[float, np.ndarray]:
        ...
        """
        (self.qm_epot, self.qm_forces) = self.calc_qm_energy_forces_MD(self.qm_potential)
        (self.epot, self.forces) = self.calc_energy_forces_MD(self.potential)
        self.epot += self.qm_epot
        self.forces += self.qm_forces

    # -----------------------------------------------------------------------------------------------------
    def calc_qm_energy_forces_MD(self, potential: str = "1") -> tuple:
        """Calculate qm energy and forces
        ...
        if potential == "1":
            ...
            self.qm_epot = a * s1 * s2 + b * y * y

        else:
            return (0.0, np.zeros(0))
            # raise ValueError(" >>> Invalid Potential!")

        self.qm_forces = torch.autograd.grad(self.epot, coords, allow_unused=True)[0]
        self.qm_forces = self.forces.detach().numpy()

        return (float(self.qm_epot), self.qm_forces)
