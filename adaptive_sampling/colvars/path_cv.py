...
class PathCV:
    ...
    def __init__(
        ...
        if guess_path == None:
            raise ValueError(
                " >>> ERROR: You have to provide a guess path to the PathCV"
            )
        ...
        if self.adaptive:
            ...
            self.weights = torch.zeros(
                self.nnodes, device=self.device, requires_grad=False
            )
            self.avg_dists = [
                torch.zeros_like(self.path[0], device=self.device, requires_grad=False)
            ...
            if half_life < 0:
                ...
                self.fade_factor = math.exp(-math.log(2.0) / float(self.half_life))
        ...
        if self.verbose:
            print(
                f" >>> INFO: Initialization of PathCV with {self.nnodes} nodes finished."
            )
    ...
    def calculate_path(self, coords: torch.tensor) -> torch.tensor:
        ...
        if self.adaptive:
            raise NotImplementedError(
                f" >>> ERROR: Adaptive path updates only implemented for gpath."
            )
    ...
    def update_path(self, z: torch.tensor, q: list = None):
        ...
        if self.n_updates < 0:
            self.weights = torch.zeros_like(
                self.weights, device=self.device, requires_grad=False
            )
            self.avg_dists = [
                torch.zeros_like(
                    self.avg_dists[0], device=self.device, requires_grad=False
                )
                for _ in range(self.nnodes)
            ]
    ...
    def reparametrize_path(
        ...
        while abs(sumlen[-1] - prevsum) > tol and iter <= max_step:
            ...
            for i, _ in enumerate(path[1:-1], start=1):
                ...
                while not ((sumlen[k] < sfrac[i]) and (sumlen[k + 1] >= sfrac[i])):
                    ...
                    if i >= nnodes:
                        raise ValueError(
                            " >>> ERROR: Reparametrization of path failed!"
                        )
                dr = sfrac[i] - sumlen[k]
                vec = path[k + 1] - path[k]
        ...
        if verbose:
            ...
            if crit < tol:
                print(
                    f" >>> INFO: Reparametrization of Path converged in {iter} steps. Final convergence: {crit:.6f}."
                )
            else:
                print(
                    f" >>> WARNING: Reparametrization of Path not converged in {max_step} steps. Final convergence: {crit:.6f}."
                )
    ...
    def smooth_string(path: list, s: float = 0.0) -> list:
        ...
        for i, coords in enumerate(path[1:-1], start=1):
            ...
            path_new[-1] += s / 2.0 * (path[i - 1] + path[i + 1])
        path_new.append(path[-1])
        return path_new
    ...
    def get_distance(
        ...
        if metric.lower() == "rmsd":
            ...
            coords_fitted, reference_fitted = kabsch_rmsd(
                coords, reference, return_coords=True
            )
    ...
    def _get_closest_nodes(
        self, z: torch.tensor, dists: list, regulize: bool = False
    ) -> tuple:
        ...
        """
        dists.insert(
            0, self.get_distance(z, self.boundary_nodes[0], metric=self.metric)
        )
        ...
        if self.closest_prev != None:
            if abs(self.closest_prev - idx0) > 1:
                ...
                if self.closest_prev == 0:
                    ...
                    d = [
                        dists[self.closest_prev - 1],
                        dists[self.closest_prev],
                        dists[self.closest_prev + 1],
                    ]
        ...
        for c in self.path:
            path_reduced.append(
                convert_coordinate_system(
                    c, self.active, coord_system=self.coordinates, ndim=self.ndim
                )
            )
        self.path = path_reduced.copy()
        if self.verbose:
            print(
                f" >>> INFO: Reduced coordinate system to {torch.numel(self.path[0])} {self.coordinates} coordinates."
            )
    ...
    def _shared_path(self) -> List[torch.tensor]:
        ...
        for i, walker in enumerate(self.walkers):
            ...
            try:
                ...
                if self.verbose:
                    print(
                        f" >>> WARNING: Failed to read {i}th walker of shared path update!"
                    )
                continue
    ...
    def _dump_update_data(self):
        ...
        for i, file in enumerate([self.local_path_file for _ in range(10)]):
            try:
                torch.save(
                    {"weights": self.weights, "path": self.path},
                    file,
                )
                break
            except:
                ...
                if i == 9 and self.verbose:
                    print(
                        f" >>> WARNING: Failed to dump multiple walker path data in `{file}`!"
                    )
                    break
                time.sleep(0.1)
                continue
    ...
    def write_path(self, filename: str = "path_cv.npy"):
        ...
        if filename[-3:] == "npy":
            ...
            for coords in self.path:
                dcd.write(
                    BOHR_to_ANGSTROM
                    * coords.view(int(torch.numel(coords) / 3), 3).detach().numpy()
                )
            dcd.close()
