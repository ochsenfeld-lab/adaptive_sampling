...
def test_init_path():
    feneb = FENEB(1.0, 10.0, ["resources/feneb_start.xyz", "resources/feneb_end.xyz"], nimages=1, convert_units=False)
    result = [
...
def test_opt_node_spacing():
    ...
    feneb.path[0] = np.asarray([[0.0,0.0,0.0]])
    feneb.path[1] = np.asarray([[0.4,0.4,0.4]])
    feneb.path[2] = np.asarray([[1.0,1.0,1.0]])
