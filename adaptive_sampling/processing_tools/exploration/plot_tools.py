...
def generate_mol_grid(df: pd.DataFrame):
    ...
    filtered_abs_prob = dict(filter(lambda elem: elem[1] > 0, abs_prob.items()))
    sorted_abs_prob = sorted(filtered_abs_prob, key=filtered_abs_prob.get, reverse=True)  # type: ignore
    ...
    for i in range(len(sorted_abs_prob)):
        if sorted_abs_prob[i] != "Revise structure":
            ...
            mol = rdmolfiles.MolFromSmiles(sorted_abs_prob[i], params)
            ...
            list_formulas.append(
                CalcMolFormula(mol) + ": " + str(abs_prob[sorted_abs_prob[i]])
            )
            list_mols.append(mol)
            rel_prob = abs_prob[sorted_abs_prob[i]]/sum(abs_prob.values())
            print(sorted_abs_prob[i] + ": " + str(abs_prob[sorted_abs_prob[i]]) + "(" + str(rel_prob) + ")")

    print("There are " + str(len(list_mols)) + " distinct molecules on the grid.\n")
    img = draw.MolsToGridImage(
        list_mols,
        molsPerRow=7,
        legends=list_formulas,
        subImgSize=[300, 300],
        maxMols=999999,
        returnPNG=False,
    )
...
def generate_network_grid(nanoNet: NanoNetwork):
    ...
    #print(nanoNet.node_labels)
    for i, node in enumerate(nanoNet.node_labels):#u_list:
        if nanoNet.node_labels[i] != "Revise structure":

            params = rdmolfiles.SmilesParserParams()
            params.removeHs = False
            params.sanitize = False
            mol = rdmolfiles.MolFromSmiles(nanoNet.node_labels[i], params)
            mol.UpdatePropertyCache(strict=False)
            rdmolops.SanitizeMol(mol, Chem.SanitizeFlags.SANITIZE_ALL ^ Chem.SanitizeFlags.SANITIZE_ADJUSTHS, catchErrors=True)
            #rdmolops.SanitizeMol(mol)
...
def plot_static_network_kk(nanoNet: NanoNetwork):
    ...
    nx.draw(
        nanoNet.graph,
        alpha=0.8,
        pos=nx.kamada_kawai_layout(nanoNet.graph),
        with_labels=True,
        labels = nanoNet.node_indices,
        linewidths=1.0,
        node_size=150,
        node_color=nanoNet.node_colors,
        edge_color=nanoNet.edge_colors,
        width=1,
        font_size=10,
        arrowsize=8,
    )
...
def plot_static_network_shell(nanoNet: NanoNetwork):
    ...
    nx.draw(
        nanoNet.graph,
        alpha=0.8,
        pos=nx.shell_layout(nanoNet.graph),
        with_labels=True,
        labels = nanoNet.node_indices,
        linewidths=1.0,
        node_size=150,
