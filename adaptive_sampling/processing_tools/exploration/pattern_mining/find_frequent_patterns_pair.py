...
species2id = {}
hyperedge2id = {}
with open(sys.argv[3]+'.trns', 'w') as out:
    out.write("")
...
for filename in os.listdir(sys.argv[1]):
    ...
    with open(sys.argv[3]+'.trns', 'a') as out:
...
for filename in os.listdir(sys.argv[2]):
    if filename[-5:] != '.json': continue
    with open(os.path.join(sys.argv[2],filename)) as json_data:
        data = json.load(json_data)
...
os.system(f"miners/lcm1 {sys.argv[3]}.trns {frequency}  > {sys.argv[3]}.frq")
...
with open(sys.argv[3]+'.frq', 'r') as file:
    for line in file:
...
with open(sys.argv[3]+'.out_1.json', 'w') as out:
    json.dump(output_1, out)
with open(sys.argv[3]+'.out_2.json', 'w') as out:
    json.dump(output_2, out)

os.system(f'rm {sys.argv[3]}.trns {sys.argv[3]}.frq')