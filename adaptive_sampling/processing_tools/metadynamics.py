        ...
        if i % hill_drop_freq == 0:
            ...
            if well_tempered_temp != None:
                w = hill_height * np.exp(
                    -metapot[grid_idx] / (kB_in_atomic * well_tempered_temp)
                )
            else:
                w = hill_height
...
def pmf_from_metapot(
    metapot: np.ndarray, well_tempered_temp: float = None, equil_temp: float = 300.0
):
    """Get potential of mean force from the metadyanamics bias potential"""
    pmf = -metapot
    if well_tempered_temp != None:
        pmf *= (equil_temp + well_tempered_temp) / well_tempered_temp
    pmf -= pmf.min()
    return pmf
